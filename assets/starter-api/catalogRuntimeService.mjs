...
export async function createCatalogRuntimeService(options) {
  ...
  return {
    ...
    getAuditEvents: (query) => persistenceStore.getAuditEvents(query),
    getProviderState: (providerId) => persistenceStore.getProviderState(providerId),
    getConnection: (providerId) => persistenceStore.getConnection(providerId),
    listConnections: () => persistenceStore.listConnections(),
    upsertConnection: (record) => persistenceStore.upsertConnection(record),
    deleteConnection: (providerId) => persistenceStore.deleteConnection(providerId),
    recordValidationRun: (result) => persistenceStore.recordValidationRun(result),
    recordAuditEvent: (event) => persistenceStore.recordAuditEvent(event),
    getPersistenceInfo: () => ({
      ...
      supportsCredentialVault: typeof persistenceStore.upsertConnection === 'function',
    }),
